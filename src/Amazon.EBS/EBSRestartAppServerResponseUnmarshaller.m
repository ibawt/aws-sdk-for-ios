...
#import "EBSRestartAppServerResponseUnmarshaller.h"

@implementation EBSRestartAppServerResponseUnmarshaller


-(EBSRestartAppServerResponse*)response {
    if( response == nil ) {
        response = [[EBSRestartAppServerResponse alloc] init];
    }
    return response;
}
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"RestartAppServerResponse"]) {
        if (caller != nil) {
