...
@implementation EBSRebuildEnvironmentResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"RebuildEnvironmentResponse"]) {
        if (caller != nil) {
...
{
    if( response == nil ) {
        response = [[EBSRebuildEnvironmentResponse alloc] init];
    }
    return response;
}
