...
@implementation EBSDescribeEventsResponseUnmarshaller
...
-(void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
    ...
    if( [elementName isEqualToString:@"Events"] ) {
        ...
        listUnmarshaller.endListElementName = @"Events";
        listUnmarshaller.entryElementName = @"member";
        listUnmarshaller.delegateClass = [EBSEventDescriptionUnmarshaller class];
...
-(void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"DescribeEventsResponse"]) {
        if (caller != nil) {
...
{
    if( response == nil ) {
        response = [[EBSDescribeEventsResponse alloc] init];
    }
    return response;
}
